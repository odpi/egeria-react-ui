...
export default function EntityNeighborhoodDiagram(props) {
  ...
  const checkFocusSet = useCallback(
   ...
   */
  const initialiseNeighborhoodDiagram = useCallback(
   ...
   */
  const entityIsTypeOf = useCallback(
  ...
  const convertCardinalityToSymbol = (card) => {
    ...
    switch (card) {
      ...
      case "ANY_NUMBER" :
        ...
        break;
      default:
        alert("Unexpected value for cardinality in EntityNeighborhoodDiagram: "+card);
        break;
    }
    return symbol;
  };
   ...
   */
  const nhbHighlight = useCallback(
   ...
   */
  const renderNeighborhoodDiagram = useCallback(
  ...
  const incrementalScroll = useCallback(
    ...
    (drg, typeName, h_pos, h, v_pos, v, tol) => {
  ...
  const nhbdUpdate = useCallback(
    ...
    [focusContext.focus, margin.left, margin.top, nhbHighlight, nodeSelected, scrollSelectedIntoView, typesContext]
  );
  ...
  useEffect(
    ...
    [ typesContext.tex, focusContext.focus, checkFocusSet, createNeighborhood,
      initialiseNeighborhoodDiagram, nhbdUpdate, renderNeighborhoodDiagram, typesContext ]
  )
